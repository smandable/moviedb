...
foreach (new DirectoryIterator($dirName) as $fileInfo) {
    ...
    $files[] = array('Title' => $fileName, 'Size' => $totalSize, 'Path' => $fileNameAndPath);
...
// var_dump($files[$keys[0]]['Title']);
// var_dump($files[$keys[1]]['Title']);
echo "\n\n";
$length = count($files);
// echo '$length: ' . $length . "\n";
$files2 = array();
//$files2[] = array('Title' => "", 'Size' => 0);
// $files2[] = array('Title' => $fileName, 'Size' => $totalSize, 'Path' => $fileNameAndPath);
...
$files2 = super_unique($files2, 'Title');
    print_r($files2);
...
for ($i=0;$i<$length;$i++) {
    ...
    // $files2[$i] = array('Title' => $nm, 'Size' => $sz);
    // //array_unique($files2);
    // print_r(super_unique($files2, 'Title'));

    // print_r(array_unique($files2));
    // if ($files2[$i]["Title"] != $files[$i]["Title"]) {
    //     echo "not equal";
    //     $files2[$i] = array('Title' => $nm, 'Size' => $sz);
    // }
    //if ($i<=$length) {
    ...
    echo "\n";
    // echo $key . ':' . $value . "\n";
    //echo $files[$keys[0]]['Title'] . "\n";

    // for ($j=1;$j<$length;$j++) {
    // $n1 = $files[$keys[$i]]['Title'];
    // echo '$n1: ' . $n1 . "\n";
    //
    // $n2 = next($files[$keys[$i]]['Title']);
    // echo  '$n2 ' . $n2 . "\n";
    //
    // echo strcasecmp($n1, $n2) . "\n";

    //if (strcasecmp($n1, $n2) == 0) {
        //echo PHP_EOL . "{$keys[0]} HAS VALUE {$files[$keys[0]]['Title']} THAT IS EQUAL TO THE VALUE OF {$files[$keys[1]]['Title']} IN {$keys[1]}";
    //}
    //}
}
